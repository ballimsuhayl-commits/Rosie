...
// --- PRODUCTION CONFIGURATION (Safe Browser Access) ---
    ...
    try {
      ...
      if (base64Audio) {
        ...
        audio.onended = () => { 
            ...
            if (isWakeWordActive) startWakeWordListener(); 
        };
        await audio.play();
      } else {
          setRosieState('default');
  ...
  const processCommand = async (text) => {
    ...
    try {
      ...
      const model = genAI.getGenerativeModel({ 
        ...
        1. If asked to go somewhere, return JSON: {"type": "navigation", "destination": "...", "overview": "...", "steps": ["Step 1", "..."]}.
        2. Otherwise, give a concise spoken reply.` 
      });
      ...
      if (reply.includes('"type": "navigation"')) {
          ...
          setCurrentNavStep(0);
          handleSpeak(`Setting off for ${nav.destination}. ${nav.overview}`);
      } else {
  ...
  const captureAndAnalyze = async () => {
      ...
      try {
          const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${config.gemini}`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ contents: [{ parts: [{ text: "Analyze this image for a family PA." }, { inlineData: { mimeType: "image/jpeg", data: img } }] }] })
          });
